package com.dx.qq.utile;

import com.dx.qq.QQUser;
import com.dx.qq.message.PictureKeyStore;
import com.dx.qq.message.PictureStore;
import com.dx.qq.sdk.*;
import com.dx.qq.window.ChatWindow;
import com.dx.qq.window.OutPutWindow;
...
public class Util {
    ...
    public static boolean isvalidimg(byte[] data) {
        ...
        try {
            log.info("记录图片。。。。");
            ImageIO.write(image, "png", new File("E:\\tup\\1.png"));
            ImageIO.write(image, "jpg", new File("E:\\tup\\2.jpg"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public static String read_property(String key) {
        File property_file = new File(Util.get_root_path() + "/dx-boot-myqq/config/record.conf");
    ...
    public static String read_config(String key) {
        File property_file = new File(Util.get_root_path() + "/dx-boot-myqq/config/Settings.conf");
        Properties properties = new Properties();
